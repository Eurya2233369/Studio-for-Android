  ...
  public void analyze(
          ...
          service.compile(
              List.of(
                  new SourceFileObject(
                      mEditor.getCurrentFile().toPath(), content.toString(), Instant.now())))) {
        diagnostics.clear();
    ...
    while (delegate.shouldAnalyze()) {
      ...
      for (Diagnostic<? extends JavaFileObject> diagnostic : diagnostics) {
        if (diagnostic.getStartPosition() <= thisIndex
            && thisIndex <= diagnostic.getEndPosition()) {
          Span span = Span.obtain(column, EditorColorScheme.COMMENT);
          span.setUnderlineColor(
              diagnostic.getKind().equals(Diagnostic.Kind.ERROR) ? 0xffFF0000 : 0xFFffff00);
