...
/**
 * CodeEditor is a editor that can highlight text regions by doing basic syntax analyzing This
...
public class CodeEditor extends View
    ...
    protected float getOffset(int line, int column) {
        return mLayout.getCharLayoutOffset(line, column)[0]
    ...
    private void initialize() {
        ...
        mDividerMargin =
                TypedValue.applyDimension(
                        TypedValue.COMPLEX_UNIT_DIP, 2, Resources.getSystem().getDisplayMetrics());
    ...
    /** Whether the editor should use a different color to draw the background of current line */
    ...
    protected void drawView(Canvas canvas) {
        ...
        boolean lineNumberNotPinned =
                isLineNumberEnabled() && (isWordwrap() || !isLineNumberPinned());
        ...
        if (lineNumberNotPinned) {
            ...
            if (firstLn != null && firstLn.value != -1) {
                ...
                if (postDrawLineNumbers.size() == 0
                        || getRowTop(IntPair.getSecond(postDrawLineNumbers.get(0))) - getOffsetY()
                                > bottom) {
        ...
        while (columnCount > span.column) {
            ...
            if (span.problemFlags > 0
                ...
                if (color != 0 && span.column >= 0 && spanEnd - span.column >= 0) {
                    ...
                    float lineWidth =
                            measureText(mBuffer, Math.max(0, span.column), spanEnd - span.column)
                                    + phi;
    ...
    protected void drawRows(
            ...
            List<CursorPaintAction> postDrawCursor,
        ...
        for (int row = getFirstVisibleRow();
                row <= getLastVisibleRow() && rowIterator.hasNext();
                row++) {
            ...
            float[] charPos =
                    findFirstVisibleChar(offset, rowInf.startColumn, rowInf.endColumn, mBuffer);
            ...
            while (temporaryOffset < getWidth() && lastVisibleChar < columnCount) {
                ...
                if (isEmoji(ch) && lastVisibleChar + 1 < columnCount) {
                    temporaryOffset +=
                            mFontCache.measureText(
                                    mBuffer, lastVisibleChar, lastVisibleChar + 2, mPaint);
                    lastVisibleChar++;
                } else {
        ...
        for (int row = getFirstVisibleRow();
            ...
            if (!mHardwareAccAllowed
                ...
                while (spanOffset + 1 < spans.size()) {
                    if (spans.get(spanOffset + 1).column <= firstVisibleChar) {
                        ...
                        if (span.problemFlags > 0
                            ...
                            float lineWidth;
                            int spanEnd =
                                    Math.min(rowInf.endColumn, spans.get(spanOffset + 1).column);
                            if (isWordwrap()) {
                                lineWidth =
                                        measureText(
                                                        mBuffer,
                                ...
                                lineWidth =
                                        measureText(mBuffer, span.column, spanEnd - span.column)
                                                + phi;
                            }
                            int waveCount = (int) Math.ceil(lineWidth / waveLength);
                            phi = waveLength - (waveCount * waveLength - lineWidth);
                ...
                while (lastVisibleChar > span.column) {
                    int spanEnd =
                            spanOffset + 1 >= spans.size()
                                    ? columnCount
                                    : spans.get(spanOffset + 1).column;
                    spanEnd = Math.min(columnCount, spanEnd);
                    ...
                    if (span.problemFlags > 0
                        ...
                        if (color != 0 && span.column >= 0 && spanEnd - span.column >= 0) {
                            ...
                            if (isWordwrap()) {
                                ...
                                startOffset =
                                        measureTextRegionOffset()
                                                + measureText(mBuffer, 0, span.column)
                                                - getOffsetX();
                                lineWidth =
                                        measureText(mBuffer, span.column, spanEnd - span.column)
                                                + phi;
                            ...
                            canvas.save();
                            canvas.clipRect(
                                    startOffset, 0, startOffset + lineWidth, canvas.getHeight());
                            canvas.translate(startOffset - phi, centerY);
                            // Draw waves
                            mPath.reset();
                            mPath.moveTo(0, 0);
                ...
                paintingOffset =
                        offset + mRenderer.drawLineHardwareAccelerated(canvas, line, offset);
            ...
            if (line == mConnection.mComposingLine) {
                ...
                if (paintStart != paintEnd) {
                    ...
                    mRect.right =
                            mRect.left + measureText(mBuffer, paintStart, paintEnd - paintStart);
            ...
            if (mCursor.isSelected()) {
                ...
                postDrawCursor.add(
                        new CursorPaintAction(
                                row,
                                centerX,
                                mEventHandler.shouldDrawInsertHandle() ? mInsertHandle : null,
                                true));
            }
        }
                ...
                if (startIndex <= selectionStart) {
                    if (endIndex >= selectionEnd) {
                        ...
                        drawText(
                                ...
                                baseline);
                        float deltaX =
                                measureText(mBuffer, startIndex, selectionStart - startIndex);
                        ...
                        drawText(
                                ...
                                baseline);
                        deltaX +=
                                measureText(mBuffer, selectionStart, selectionEnd - selectionStart);
                        ...
                        drawText(
                                ...
                                offsetX
                                        + measureText(
                                                mBuffer, startIndex, selectionStart - startIndex),
                                baseline);
                    }
                } else {
                    ...
                    if (endIndex > selectionEnd) {
                        ...
                        drawText(
                                ...
                                offsetX
                                        + measureText(
                                                mBuffer, startIndex, selectionEnd - startIndex),
                                baseline);
    ...
    * Draw Error of this
    * 
    * @param List<String> is Error Masseage
    * @param index is cursor left
    */
    
    private void drawErrorMsg(List<String> msg, int index) {
    ...
    /**
     * Draw a handle. The handle can be insert handle,left handle or right handle
     *
    ...
    protected void drawHandle(
            Canvas canvas, int row, float centerX, RectF resultRect, int handleType) {
                    ...
                    mRect.bottom =
                            Math.min(
                                    ...
                                    (block.toBottomOfEndLine
                                                    ? getRowBottom(block.endLine)
                                                    : getRowTop(block.endLine))
                                            - getOffsetY());
    ...
    protected void drawCursor(
            Canvas canvas, float centerX, int row, RectF handle, boolean insert, int handleType) {
        if (!insert || mCursorBlink == null || mCursorBlink.visibility) {
            mRect.top = getRowTop(row) - getOffsetY();
            mRect.bottom = getRowBottom(row) - getOffsetY();
            mRect.left = centerX - mInsertSelWidth / 2f;
    ...
    protected float measureText(char[] src, int index, int count) {
        ...
        float extraWidth =
                mFontCache.measureChar(' ', mPaint) * getTabWidth()
                        - mFontCache.measureChar('\t', mPaint);
    ...
    protected void drawText(
            Canvas canvas, char[] src, int index, int count, float offX, float offY) {
        int end = index + count;
        if (mCharPaint) {
    ...
    protected float[] findFirstVisibleChar(
            float initialPosition, int left, int right, char[] chars) {
        float width = 0f;
    ...
    /**
     * Draw rect on screen Will not do any thing if color is zero
     *
     * @param canvas Canvas to draw
    ...
    public void setTypefaceText(Typeface typefaceText) {
        ...
        if (2 * mPaint.measureText("/") != mPaint.measureText("//")) {
            Log.w(
                    LOG_TAG,
                    "Font issue:Your font is painting '/' and '//' differently, which will cause the editor to render slowly than other fonts.");
            mCharPaint = true;
        } else {
            mCharPaint = false;
        }
    ...
    public void moveSelectionDown() {
        if (mLockedSelection == null) {
            ...
            long pos =
                    mCursor.getDownOf(IntPair.pack(mCursor.getLeftLine(), mCursor.getLeftColumn()));
    ...
    public void moveSelectionUp() {
        if (mLockedSelection == null) {
            ...
            long pos =
                    mCursor.getUpOf(IntPair.pack(mCursor.getLeftLine(), mCursor.getLeftColumn()));
            setSelection(IntPair.getFirst(pos), IntPair.getSecond(pos));
        } else {
            mCompletionWindow.hide();
    ...
    protected void updateSelection() {
        ...
        // Logs.log("Send selection changes: candidate(start = " + candidatesStart + ", end =" +
        // candidatesEnd + " ) cursor(left = " + mCursor.getLeft() + ", right =" +
        // mCursor.getRight() + ")");
    ...
    public void afterInsert(
        ...
        invalidateInCursor();
        if (mListener != null) {
            mListener.afterInsert(
                    this, mText, startLine, startColumn, endLine, endColumn, insertedContent);
        }
    }
    ...
    public void afterDelete(
        ...
        if (mListener != null) {
            mListener.afterDelete(
                    this, mText, startLine, startColumn, endLine, endColumn, deletedContent);
        }
    }
    ...
    public void onFormatFail(final Throwable throwable) {
        if (mListener != null && !mListener.onFormatFail(this, throwable)) {
            post(
                    () ->
                            Toast.makeText(getContext(), throwable.toString(), Toast.LENGTH_SHORT)
                                    .show());
        }
        mFormatThread = null;
    }
                    ...
                    () -> {
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
                            mRenderer.invalidateDirtyRegions(
                                    provider.mObjContainer.spanMap,
                                    provider.getResult().getSpanMap());
                        }
                        invalidate();
                    });
        }
